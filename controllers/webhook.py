    ...
    def _process_message(self, bot, data):
        ...
        try:
            ...
            if participant and participant.current_step_id and (text or contact):
                ...
                if contact:
                    ...
                    if str(contact_user_id) != str(telegram_id):
                        service = request.env['telegram.service'].sudo().with_context(bot_id=bot.id).new()
                        service.send_message(chat_id=str(chat_id), message='لطفاً شماره تماس خود را به اشتراک بگذارید')
                        return Response(status=200)
            ...
            if not (text and text.startswith('/start')):
                service = request.env['telegram.service'].sudo().with_context(bot_id=bot.id).new()
                service.send_message(chat_id=chat_id, message='درخواست شما نامعتبر است.')
            return Response(status=200)
